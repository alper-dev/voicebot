...
class ACAR extends Client {
    constructor(options) {
        ...
        this.urlStaff = Settings.youtube_staff
        this.stream;
        this.message;
        this.channelId;
        this.playing;
    ...
    async _playStaff(channelId, a) {
        ...
        let stream;
        let resource;
        if (Settings.local_staff) {
            resource = this.stream = createAudioResource(Settings.file_staff);
        } else {
            stream = await play.stream(this.urlStaff);
            resource = this.stream = createAudioResource(stream.stream, {
                inputType: stream.type,
            });
        }
        ...
        player.on(AudioPlayerStatus.Idle, async () => {
            ...
            if (Settings.local_staff) {
                ...
                stream = await play.stream(this.url_staff);
                resource = this.stream = createAudioResource(stream.stream, {
                    inputType: stream.type,
                });
            }
